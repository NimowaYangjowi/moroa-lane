...
완성형 쇼핑몰이 아니라 연동 설명용 데모다. 사용자가 보는 화면은 실제 DTC 커머스 사이트처럼 보여야 하지만, 실제 목적은 채널톡이 고객을 식별하고 상담 맥락을 붙이는 과정을 보여주는 것이다.

사이트의 완성도 기준은 “진짜 결제까지 되는가?”가 아니다. “면접관이 봤을 때 실제 고객사가 운영하는 쇼핑몰에 채널톡을 붙인 장면으로 보이는가?”가 기준이다.
...
- 홈: 스킨케어 브랜드의 첫 화면이다. 브랜드 이름, 대표 상품, 베스트 상품, 카테고리 진입점이 보여 실제 쇼핑몰처럼 느껴져야 한다.
- 상품 목록: 고객이 여러 상품을 훑어보는 화면이다. 카드, 가격, 카테고리, 상품 이미지가 있어야 한다.
...
- 첫 화면과 상품 화면이 실제 커머스 예제 사이트처럼 보인다.
