...
화면 목표는 실제 DTC 커머스 사이트처럼 보이는 예제 사이트다. 면접관이 브라우저를 봤을 때 “실제 고객사가 운영하는 쇼핑몰에 채널톡을 붙인 상황”을 바로 떠올릴 수 있어야 한다. 다만 운영용 커머스 기능을 완성하는 것이 아니라, 고객 식별과 상담 맥락 전달을 설명할 수 있을 만큼만 구현한다.
...
- 실제 사이트처럼 보이는 홈, 상품 목록, 상품 상세, 장바구니, 마이페이지 흐름을 보여준다.
...
기능을 추가할 때마다 “이 기능이 채널톡 면접 데모에서 어떤 상담 장면을 설명해 주는가?”를 확인한다. 쇼핑몰 기능 자체가 목적이 아니라, 실제 고객사 사이트에서 채널톡이 어떤 고객 정보를 받는지 보여주는 것이 목적이다.
